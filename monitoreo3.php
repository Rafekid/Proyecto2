<?php
    ...
    $result = $conexion->query("select sum(monto) as acumulado,fecha from transaction where fecha=curdate()") or die($conexion->error());
    $row = mysqli_fetch_assoc($result);
    $cantidad = $row['acumulado'];
    $fecha = $row['fecha'];

    /* print_r($fecha); */    

    $sql="SELECT sum(monto), fecha from transaction where fecha=curdate()";
...
<body>
    ...
    Bienvenido al portal de Administrador <?php echo $usuario2?></div>
        ...
        <div class="contenedor">
            ...
            <div class="contenido">
                <div class="alert alert-success" role="alert">
                 Se acumuló un monto de : <?php echo $cantidad;?> el día de hoy : <?php echo $fecha;?></div>
